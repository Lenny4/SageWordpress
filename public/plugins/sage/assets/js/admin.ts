...
$(() => {
  ...
  let translationString = $("[data-sage-translation]").attr('data-sage-translation');
  let translations: any = [];
  if (translationString) {
    translations = JSON.parse(translationString);
  }
  let translationJs: any = $("[data-sage-js-translation]").attr('data-sage-js-translation');
  if (translationJs) {
  ...
  function addFilter() {
    ...
    let newFilterContainer = $('<div class="filter-container" style="margin-bottom: 5px;display: flex;flex-wrap: wrap;"></div>').appendTo(allFilterContainer);

    let chooseFieldContainer = $('<div></div>').appendTo(newFilterContainer);
    let chooseFieldLabel = $('<label class="screen-reader-text" for="filter_field[' + index + ']">filter_field[index]</label>').appendTo(chooseFieldContainer);
    let chooseFieldSelect = $('<select name="filter_field[' + index + ']" id="filter_field[' + index + ']"></select>').appendTo(chooseFieldContainer);
    let chooseFieldOptionDefault = $('<option disabled selected value> -- select a field -- </option>').appendTo(chooseFieldSelect);
    for (let field of allFields) {
      ...
      let chooseFieldOption = $('<option value="' + field.name + '">' + fieldName + '</option>').appendTo(chooseFieldSelect);
    }

    let chooseFilterTypeContainer = $('<div></div>').appendTo(newFilterContainer);
    let chooseFilterTypeLabel = $('<label class="screen-reader-text" for="filter_type[' + index + ']">filter_type[index]</label>').appendTo(chooseFilterTypeContainer);
    let chooseFilterTypeSelect = $('<select disabled name="filter_type[' + index + ']" id="filter_type[' + index + ']"></select>').appendTo(chooseFilterTypeContainer);
    let chooseFilterTypeOptionDefault = $('<option disabled selected value></option>').appendTo(chooseFilterTypeSelect);

    let chooseValueContainer = $('<div></div>').appendTo(newFilterContainer);
    let chooseValueLabel = $('<label class="screen-reader-text" for="filter_value[' + index + ']">filter_value[index]</label>').appendTo(chooseValueContainer);
    let chooseValueInput = $('<input disabled type="search" id="filter_value[' + index + ']" name="filter_value[' + index + ']" value="">').appendTo(chooseValueContainer);

    let deleteField = $('<span data-delete-filter class="dashicons dashicons-trash button" style="padding-right: 22px"></span>').appendTo(newFilterContainer);
    index++;

    if (getNumberFilter() > 1 && $('#where_condition').length !== 1) {
      let orAndSelect = $('<select name="where_condition" id="where_condition"></select>').appendTo('#or_and_container');
  ...
  function validateForm() {
    ...
    $('.filter-container').each((function (index, filterContainer) {
      let chooseFieldSelect = $(filterContainer).find('select[name^="filter_field["]');
      let chooseFilterTypeSelect = $(filterContainer).find('select[name^="filter_type["]');
      let chooseValueInput = $(filterContainer).find('input[name^="filter_value["]');
      ...
      if (chooseFieldSelectVal == null) {
        $(chooseFieldContainer).append('<p class="error-message">Please select a value</p>');
      } else if (chooseValueInputVal === '') {
        $(chooseValueContainer).append('<p class="error-message">This field must not be empty</p>');
      }
    }));
  ...
  function onChangeField(container: JQuery) {
    let allFilterType = JSON.parse($('[data-all-filter-type]').attr("data-all-filter-type"));
    let allFields = JSON.parse($('[data-all-fields]').attr("data-all-fields"));

    let field = $(container).find('select[name^="filter_field"]').val();
    let chooseFilterTypeSelect = $(container).find('select[name^="filter_type"]');
    ...
    $(chooseFilterTypeSelect).find('option').each((function (index, option) {
    ...
    if (JSON.stringify(oldOptions) !== JSON.stringify(newOptions)) {
      ...
      for (let option of newOptions) {
        ...
        let chooseTypeOption = $('<option value="' + option + '">' + typeName + '</option>').appendTo(chooseFilterTypeSelect);
      }
    }
  ...
  function applySelectedValue(container: JQuery) {
    let chooseValueInput = $(container).find('input[name^="filter_value"]');
    let chooseValueSelectInput = $(container).find('select[data-filter-value-select]');
    $(chooseValueInput).val($(chooseValueSelectInput).val())
  }

  function _displayOptionWithOptGroup(availableValues: any, dom: JQuery, selectedValues: any) {
    for (const key in availableValues) {
      ...
      if (typeof availableValues[key] === "string") {
        const optionDom = $('<option ' + selected + ' value="' + key + '">[' + key + ']: ' + availableValues[key] + '</option>').appendTo(dom);
      } else {
  ...
  function showHideAvailableValues(container: JQuery) {
    ...
    if (availableValues !== null) {
      const chooseValueSelectInput = $('<select data-filter-value-select ' + multiple + '></select>').appendTo(chooseValueContainer);
  ...
  function initFiltersWithQueryParams() {
    ...
    for (const obj of Object.entries(filters)) {
      ...
      let chooseFieldSelect = $(newFilterContainer).find('select[name^="filter_field["]');
      let chooseFilterTypeSelect = $(newFilterContainer).find('select[name^="filter_type["]');
      let chooseValueInput = $(newFilterContainer).find('input[name^="filter_value["]');
  ...
  $(document).on('input', '[name="sage-fdocentete-dopiece"]', function (e) {
    ...
    setTimeout(async () => {
      ...
      const response = await fetch(siteUrl + "/index.php?rest_route=" + encodeURI("/sage/v1/fdocentetes/" + encodeURIComponent(currentSearch)) + "&_wpnonce=" + $(inputWpnonce).val());
      if (currentSearch !== searchFDocentete) {
        return;
      }
      ...
      if (response.status === 200) {
        ...
        try {
          ...
          const errorDiv = $("<div class='notice notice-error'></div>").prependTo(domContainer);
          $(errorDiv).html('<pre>' + JSON.stringify(body, undefined, 2) + '</pre>');
        } catch (e) {
          console.error(e);
        }
      }
  ...
  $(document.body).on('click', 'a[href*="page=sage_"]', function (e) {
    const defaultFilters = JSON.parse($("[data-sage-default-filters]").attr('data-sage-default-filters'));
    const url = URL.parse($(e.target).attr("href"), $("[data-sage-admin-url]").attr('data-sage-admin-url'))
    let page = null;
  ...
  function wcFreeShippingShowHideMinAmountField(el: JQuery) {
    ...
    const minAmountField = $('[id^="woocommerce_"][id$="_min_amount"]', form).closest('fieldset');
    const minAmountFieldLabel = minAmountField.prev();

    const ignoreDiscountField = $('[id^="woocommerce_"][id$="_ignore_discounts"]', form).closest('fieldset');
  ...
  $(document.body).on('change', '[id^="woocommerce_"][id$="_requires"]', function () {
