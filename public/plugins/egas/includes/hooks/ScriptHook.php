...
class ScriptHook
{
    ...
    private ?string $scriptSuffix = null;
    private ?string $version = null;

    public function __construct()
    {
        add_action('wp_enqueue_scripts', function (): void {
            [$assetsDistUrl, $scriptSuffix, $version] = $this->getData();
    ...
    private function getData(): array
    {
        if (is_null($this->assetsDistUrl)) {
            ...
            $this->scriptSuffix = esc_url(trailingslashit(plugins_url('/dist/', $sage->file)));
            $pluginData = get_plugin_data($sage->file);
            $this->version = $pluginData['Version'];
        }
        return [$this->assetsDistUrl, $this->scriptSuffix, $this->version];
    }
}
